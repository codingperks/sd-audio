                ...
                wandb.log(
                    {
                        "train_input/training_images_audio (LOSSY)": wandb.Audio(
                            preprocessor.spec_to_wav_np(image),
                            sample_rate=44100,
                            caption=f"Epoch {epoch} Step {step}",
                        )
                    },
            ...
            for step, batch in enumerate(val_dataloader):
                ...
                with torch.no_grad():  # disable gradient calculation
                    ...
                    val_avg_loss = accelerator.gather(
                        val_loss.repeat(args.val_batch_size)
                    ).mean()
        ...
        if accelerator.is_main_process:
            if (
                ...
                validation_prompts = [
                    ...
                    args.validation_prompt2,
                    args.validation_prompt3,
                    args.validation_prompt4,
                    args.validation_prompt5,
                ]
                images = []
                audios = []
                image_prompts = []
